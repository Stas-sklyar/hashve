...
import {TranslateService as NGXTranslateService, MissingTranslationHandler, MissingTranslationHandlerParams} from '@ngx-translate/core';
...
const LANG_LIST: ILang[] = [
  {code: 'heb', name: 'עברית', culture: 'heb-IL'},
...
export class TranslatesService {
  constructor(
    ...
    @Inject('ORIGIN_URL') private _host: string,
  ) {
    // console.log(_host);
...
export class CommonMissingTranslationHandler implements MissingTranslationHandler {
  handle(params: MissingTranslationHandlerParams) {
    if (
      ...
      params.translateService.translations['heb'] &&
      !params.translateService.translations['heb'][params.key]
    ) {
